...
namespace RemoteSupport.Server
{
    public class Receiver
    {
        ...
        /// <summary>
        /// The real Socket working in the background.
        /// </summary>
        ...
        /// </summary>
        /// <param name="client">The Socket Client  to encapsulate that was obtained by the Netwrok Stream.</param>
        /// <param name="server">The reference to the parent server containing the receivers list.</param>
        public Receiver(Socket client, Server server)
            : this()
        {
            Server = server;
            Client = client;
        ...
        /// <summary>
        /// Stops all data transmition and disconnectes the Socket Client
        /// </summary>
            ...
            while (Status != StatusEnum.Disconnected)
            {
                if (MessageQueue.Count > 0)
                {
                    ...
                    try
                    {
                        ...
                        f.Serialize(new NetworkStream(Client),message);
                    }
                    catch
                    {
                        Disconnect();
        ...
        private void ReceivingMethod()
        {
            while (Status != StatusEnum.Disconnected)
            {
                if (Client.Available > 0)
                {
                    ...
                    {
                        BinaryFormatter f = new BinaryFormatter();
                        MessageBase msg = f.Deserialize(new NetworkStream(Client)) as MessageBase;
